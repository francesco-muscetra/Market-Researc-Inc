...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        echo "<h1>Maggiore importazione di $opzione_selezionata per abitante</h1>";
    }
    ?>
    ...
    <table>
        <tr>
            <th>Nazione</th>
            <th>Massima Quantita' Importata</th>
        </tr>
        <?php
        ...
        $sql = "SELECT n.nomeNazione AS CountryCode, MAX(c.quantitaimportata/ n.numAbitanti) AS MaxQuantitaImportata
        FROM nazioni n, prodotti p, consumazioni c
         WHERE n.codNazione = c.codNazione AND p.codProdotto = c.codProdotto AND p.nomeProdotto='$opzione_selezionata'
        AND c.quantitaimportata / n.numAbitanti = (
            SELECT MAX(c1.quantitaimportata / n1.numAbitanti)
            FROM consumazioni c1, nazioni n1
            WHERE c1.codProdotto = p.codProdotto AND c1.codNazione = n1.codNazione
        )
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>".$row["MaxQuantitaImportata"]." KG</td>
                      </tr>";
            }
        } else {
            echo "0 risultati";
